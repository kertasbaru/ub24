...
username=$(curl -s https://raw.githubusercontent.com/kertasbaru/vps_access/main/ip | grep "$Ip_Vps" | awk '{print $2}')
echo "$username" > /usr/bin/user

expx=$(curl -s https://raw.githubusercontent.com/kertasbaru/vps_access/main/ip | grep "$Ip_Vps" | awk '{print $3}')
echo "$expx" > /usr/bin/e
...
Exp1=$(curl -s https://raw.githubusercontent.com/kertasbaru/vps_access/main/ip | grep "$Ip_Vps" | awk '{print $4}')
if [[ "$today" < "$Exp1" ]]; then
    sts="${GREEN}Active${NC}"
else
    sts="${RED}Expired${NC}"
...
function restart_system() {
    ...
    local USRSC=$(wget -qO- https://raw.githubusercontent.com/kertasbaru/vps_access/main/ip | grep "$ipsaya" | awk '{print $2}')
    local EXPSC=$(wget -qO- https://raw.githubusercontent.com/kertasbaru/vps_access/main/ip | grep "$ipsaya" | awk '{print $3}')
...
function udp_mini() {
    ...
    wget -q https://raw.githubusercontent.com/kertasbaru/Mamardashvili/main/files/limit-quota.sh -O /root/limit-quota.sh
    ...
    wget -q https://raw.githubusercontent.com/kertasbaru/Mamardashvili/main/lock-service.sh -O /root/lock-service.sh
